...
import { getPuzzleById, submitPuzzleSolution, awardXP } from '@/app/actions/puzzles';
...
export default function PuzzleDetail() {
    ...
    useEffect(() => {
        if (user && puzzle && user.completed_puzzles?.includes(puzzle.id)) {
    ...
    if (loading) {
        return (
            <div className="flex flex-col items-center justify-center h-[60vh] gap-4">
                <Loader2 className="w-12 h-12 text-decode-accent animate-spin" />
                <p className="text-decode-text-secondary font-mono">Initializing kernel...</p>
            </div>
        );
    }
    ...
    const validateSolution = (userCode: string, puzzleData: any) => {
        // Since we don't have a real C backend yet, we'll simulate validation
        // using keyword pattern matching based on the puzzle description/expected logic.

        const codeClean = userCode.toLowerCase();

        // Example validation logic based on puzzle content
    ...
    const handleSubmit = async () => {
        ...
        try {
            ...
            if (result.success) {
                ...
                // 3. Award XP and Refresh Profile
                if (!isSolvedLocally) {
